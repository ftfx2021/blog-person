...
//                ),
    ...
    public Page<ArticleDocument> advancedSearch(
        ...
        //=====================================查询构建============================================
        //构建按照权重的多条件关键词查询
        MultiMatchQuery keyWordQuery = MultiMatchQuery.of(b -> b
                .query(keyword)
                .fields("title^3", "content", "summary^2"));

        //构建按照分类查询
        TermQuery categoryNameQuery = TermQuery.of(b -> b
                .field("categoryName")
                .value(categoryName));

        //构建按照tag查询
        TermsQuery tagsQuery = TermsQuery.of(b -> b.field("tags")
                .terms(tv -> tv.value(tags.stream()//传入List<FieldValue>类型，如FieldValue.of(1),FieldValue.of("str")
                        .map(FieldValue::of)
                        .collect(Collectors.toList()))));

        //构建按照status查询(过滤)
        TermQuery statusQuery = TermQuery.of(b -> b.field("status").value(1));


        //=====================================排序构建============================================
        //按相关性排序建立SortOption
        SortOptions sortScoreOptions = SortOptions.of(so->so.score(ScoreSort.of(scoreSort -> scoreSort.order(SortOrder.Desc))));
        //按创建时间排序建立SortOption
        FieldSort createTimeSort = FieldSort.of(f -> f.field("createTime").order(SortOrder.Desc));
        SortOptions createTimeSortOptions = SortOptions.of(so -> so.field(createTimeSort));
        ...
        Highlight highlightConfig = Highlight.of(h -> h
                .preTags("</em>")
                .postTags("</em>")
                .type(HighlighterType.Unified)//默认最智能
                .encoder(HighlighterEncoder.Default) //默认不对html进行转译
                .fields("title", hf -> hf)
                .fields("content", hf -> hf)
                .fields("summary", hf -> hf));//hf->hf直接返回，使用全局配置，不做任何单独的配置
        ...
        SearchResponse<ArticleDocument> response = client.search(s -> s
                        ...
                        .query(q -> q
                                .bool(b -> {
                                    ...
                                    if (keyword != null && !keyword.isEmpty()) {
                                        b.should(keyWordQuery);
                                    }

                                    // 2. 分类过滤（可选）- 使用filter，精确匹配
                                    if (categoryName != null && !categoryName.isEmpty()) {
                                        b.filter(categoryNameQuery);
                                    }
                        ...
                        .size(size)
                        // 按相关性和时间排序
                        .sort(sortScoreOptions)
                        .sort(createTimeSortOptions)
                        .highlight(highlightConfig),  //hf->hf直接返回，使用全局配置，不做任何单独的配置
                //==============================ES9语法============================//
//                        .highlight(h -> h
//                                .preTags("<em>")
//                                .postTags("</em>")
