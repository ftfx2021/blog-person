...
public class ModelHealthStore {
    ...
    public boolean isUnavailable(String id){
        ModelHealth modelHealth = healthById.get(id);
        if(modelHealth==null){
            return false;
        }
        if(modelHealth.getState().equals(State.OPEN)&&modelHealth.getOpenUntil()>System.currentTimeMillis()){
            return true;
        }
        return modelHealth.getState().equals(State.HALF_OPEN)&&modelHealth.isHalfOpenInFlight();
    }
