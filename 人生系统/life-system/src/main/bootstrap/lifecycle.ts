...
export function registerApplicationLifecycle(application: App): void {
  void application.whenReady().then(() => {
    ...
    void loadConnection().then(async(configuration) => {
      if (!configuration) return
      // 启动时先检测并补齐迁移，再开放连接池；失败保持设置页可用且不让应用崩溃。
      await runMigrations(configuration, application.isPackaged ? join(process.resourcesPath, 'migrations') : join(application.getAppPath(), 'migrations'))
