...
export async function configurePool(configuration: MysqlConnectionConfiguration): Promise<Pool> {
...
export function requirePool(): Pool {
  if (!activePool) throw Object.assign(new Error('请先在设置页配置 MySQL 连接'), { code: 'DB_UNAVAILABLE' })
  return activePool
}
