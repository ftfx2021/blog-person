...
export const mysqlConnectionSchema = z.object({
  ...
  password: z.string(),
  database: z.string().trim().regex(/^[a-zA-Z0-9_]+$/, '数据库名只能包含字母、数字和下划线'),
  connectTimeout: z.number().int().min(1000).max(30000).default(5000)
})

export type MysqlConnectionConfiguration = z.infer<typeof mysqlConnectionSchema>

export function parseMysqlUrl(url: string): MysqlConnectionConfiguration {
  const parsed = new URL(url)
  if (parsed.protocol !== 'mysql:') throw new Error('连接地址必须使用 mysql:// 协议')
